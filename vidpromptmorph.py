    ...
    def run(self, p, input_path, output_path, crf, fps, start_time, end_time, show_preview, start_prompt, end_prompt):
        ...
        for readoutimage in lengthtest:
            ...
            for _ in range(p.batch_size):
                ...
                #if len(np_image)==0:
                #    break;
                PIL_image = Image.fromarray(np.uint8(readoutimage).reshape((p.height,p.width, 3)), mode="RGB" )
            ...
            res_indexes, prompt_flat_list, prompt_indexes = prompt_parser.get_multicond_prompt_list([start_prompt, end_prompt])
